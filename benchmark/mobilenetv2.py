...
def block_mobilenet(pretrained=False, num_classes=1000, bfp=False, 
    ...
    if (bfp):
        ...
        block_model, weight_exp_list = mobilev2_converter(golden_model, block_model, group, is_kl=False)  
        model = block_model 
    else:
        model = golden_model
...
class block_InvertedResidual(nn.Module):
    ...
    def forward(self, x):
        if self.use_res_connect:
            ...
            in_exp = Utils.find_max_exponent(in_exp, quant_dim=len(x.shape)-1)
            in_exp = Utils.find_max_exponent(in_exp, quant_dim=len(x.shape)-2)
            ...
            x = BFPActivation.transform_activation_offline(x, self.exp_bit, self.mantisa_bit, max_exp)
